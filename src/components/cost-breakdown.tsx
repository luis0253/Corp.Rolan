...
export function CostBreakdown({ result, isLoading, setQuoteResult }: CostBreakdownProps) {
  ...
  const handleSave = async () => {
    ...
    setIsExporting(true); // reutiliza el estado como isSaving
    try {
      await saveQuoteToHistory(result, clientName);
      toast({ title: "✓ Cotización guardada correctamente." });
      setClientName('');
    } catch (err) {
    ...
    <Card className="shadow-lg">
      ...
      <CardFooter className="flex-col items-stretch gap-4 rounded-b-lg bg-muted p-4 mt-6">
        ...
        <Button onClick={handleSave} disabled={isExporting}>
          ...
          {isExporting ? "Guardando..." : "Guardar Cotización"}
        </Button>
        <p className="text-xs text-center text-muted-foreground">*Cotización estimada. Los costos pueden variar.</p>
      </CardFooter>
    </Card>
  );
}
